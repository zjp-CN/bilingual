...
#[test]
fn tencent_test() -> tencent::MultiErrResult<()> {
    use tencent::{HeaderJson, Query, User};
    ...
    assert_display_snapshot!(header.signature()?, @"21a54f50abc222414ea7df403a77b651b071af5c21a72b1f30b374f0cfbfa478");
    ...
    assert_display_snapshot!(header.authorization()?,
    @"TC3-HMAC-SHA256 Credential=0/2021-11-05/tmt/tc3_request,SignedHeaders=content-type;host,Signature=21a54f50abc222414ea7df403a77b651b071af5c21a72b1f30b374f0cfbfa478");
    Ok(())
}

#[test]
fn compare_test() -> tencent::MultiErrResult<()> {
    use hmac::{Mac, NewMac};
    use tencent::{hash256_string, HmacSha256};
    ...
    assert_display_snapshot!(hash256_string(test.as_bytes())?,
    ...
    assert_display_snapshot!(hash256_string(b"a")?,
    @"3acdaa86b3d73e8d18b7019d3f520000531a23db3b6dda7a94ad28db61a9008c");
    let mac = HmacSha256::new_from_slice(b"a")?;
...
#[test]
fn sha256_test() {
    use tencent::{hash256, hash256_string};
    ...
    assert_eq!(hash256(data), "031edd7d41651593c5fe5c006fa5752b37fddff7bc4e843aa6af0c950f4b9406");
    assert_display_snapshot!(hash256_string(data).unwrap(),
    @"f5d6645a2c7bc9a9f1ffffe0931ae24c4dd37904cdab576bc4147ef1b0441a9f"); // 不需要的
...
#[test]
fn detaild_hexhash_in_signature_and_authorization() -> tencent::MultiErrResult<()> {
    use tencent::{hash256, hash_2u8, hash_hash_u8, hmac_sha256_string};
...
fn serde_json_format_test() -> serde_json::Result<()> {
    ...
    let query = tencent::Query { from:      "en",
                                 to:        "zh",
                                 projectid: 0,
                                 q:         &["hi", "there"], };
